...
#include "com_microchip_grc_Emulator.h"
...
char* getString(JNIEnv *env, jobject string) {
    const char *_chars = (*env)->GetStringUTFChars(env, string, NULL);
    jsize size = (*env)->GetStringUTFLength(env, string) + 1;
    char *chars = malloc(size);
    memset(chars, 0, size);
    memcpy(chars, _chars, size);

    (*env)->ReleaseStringUTFChars(env, string, _chars);
    return chars;
}
...
GFX_STATUS GFX_ExternalResourceCallback(
    ...
    resource_file = fopen(fileName, "r");
